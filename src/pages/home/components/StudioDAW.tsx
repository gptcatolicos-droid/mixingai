          ...
          <div style={{flex:1,display:'flex',minHeight:0,overflow:'hidden'}}>
            ...
            <div style={{flex:1,overflow:'auto',position:'relative',background:'rgba(8,4,12,0.5)'}}>
              ...
              <div style={{position:'relative',minWidth:'max-content',minHeight:stems.length*TH+TH}}>
                ...
                {stems.length>0&&(
                  <div style={{height:TH,display:'flex',alignItems:'center',padding:'0 6px'}}>
                    <div style={{display:'inline-flex',alignItems:'center',gap:6,padding:'6px 14px',border:`1px dashed ${T.borderStrong}`,borderRadius:7,color:T.pink,fontSize:10,fontWeight:500,background:'rgba(192,38,211,0.04)',cursor:'pointer'}} onClick={()=>onNavigate?onNavigate('create'):null}>
                      ✦ Crear con IA — agrega instrumentos al DAW
                    </div>
                  </div>
                )}
                {/* Playhead */}
