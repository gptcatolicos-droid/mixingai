...
type Screen = 'home'|'studio'|'mixer'|'create'|'separate'|'stems'|'mixsong'|'export';
...
export default function ProjectDashboard() {
  ...
  // Sin usuario → landing
  if (!user) {
    return <HomeHero onStartMixer={(preset, files, mode) => {
      setActivePreset(preset);
      setUploadedFiles(files);
      setProjectId(newId());
  ...
  // Home - tablero
  return <FlowHome user={user!} onLogout={handleLogout} onNavigate={(id, files, presetId, mode) => {
