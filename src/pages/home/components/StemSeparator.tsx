...
const QUALITY_OPTIONS = [
  { id: 'fast', label: 'Rápido',   time: '~30 seg', desc: 'Buena calidad' },
  { id: 'std',  label: 'Estándar', time: '~90 seg', desc: 'Recomendado' },
  { id: 'hd',   label: 'HD',       time: '~3 min',  desc: 'Máxima calidad' },
];
const T = {
...
function float32ToWav(samples: Float32Array, sr: number): Blob {
  ...
  wr(0, 'RIFF'); v.setUint32(4, 36 + dl, true); wr(8, 'WAVE'); wr(12, 'fmt ');
  ...
  for (let i = 0; i < samples.length; i++, o += 2)
    v.setInt16(o, Math.max(-32768, Math.min(32767, Math.round(samples[i] * 32767))), true);
...
function base64ToFloat32(b64: string): Float32Array {
  ...
  const offset = 44; // skip WAV header
  const numSamples = Math.floor((bytes.length - offset) / 2);
  const samples = new Float32Array(numSamples);
  for (let i = 0; i < numSamples; i++) samples[i] = view.getInt16(offset + i * 2, true) / 32768;
  return samples;
}
...
function fmtDur(samples: number, sr: number) {
...
export default function StemSeparator({ user, onBack, onCreditsUpdate, onStemsReady }: StemSeparatorProps) {
  ...
  const [serverMode, setServerMode] = useState<'server' | 'local' | null>(null);
  ...
  const handleSeparate = async () => {
    ...
    setError(''); setPhase('decoding'); setProgress(5); setProgressText('Decodificando audio...');
    try {
      const { data, sampleRate } = await decodeAudioFile(file);
      setProgress(20); setProgressText('Preparando para enviar al servidor...');
      setPhase('processing');

      // Convertir a WAV base64
      ...
      // Intentar primero con el servidor (RunPod via Edge Function)
      if (token) {
        setProgress(35); setProgressText('Conectando con servidor RunPod...');
        ...
        try {
          const resp = await fetch(`${SUPABASE_URL}/functions/v1/stems-separate`, {
            ...
            body: JSON.stringify({ audioBase64, mimeType: 'audio/wav', quality }),
            signal: AbortSignal.timeout(300000),
          });
          const result = await resp.json();

          if (resp.ok && result.success && result.stems) {
            // Servidor respondió con los stems
            setProgress(90); setProgressText('Exportando stems...');
            const stemResults: StemResult[] = STEM_META.map((meta) => {
              const b64data = result.stems[meta.key];
              const pcm = base64ToFloat32(b64data);
              const blob = float32ToWav(pcm, SAMPLE_RATE);
              const url = URL.createObjectURL(blob);
              return { ...meta, blob, url, duration: fmtDur(pcm.length, SAMPLE_RATE) };
            });
            setStems(stemResults); setPhase('done'); setProgress(100);
            onCreditsUpdate(result.creditsRemaining ?? user.credits - 3);
            return;
          }

          // Si el servidor no tiene Demucs o hay error, hacer fallback local
      ...
      // Fallback: separación básica local
      setProgress(40); setProgressText('Servidor no disponible · Separando localmente...');
      
      // Separación básica por bandas de frecuencia
      const len = data.length;
      const stemsData: Record<string, Float32Array> = {
      ...
      // Simulación de separación por energía
      for (let i = 0; i < len; i++) {
        const s = data[i];
        stemsData.bass[i] = s * 0.9;
        stemsData.drums[i] = Math.abs(s) > 0.3 ? s * 0.8 : s * 0.05;
        stemsData.vocals[i] = s * 0.7;
        stemsData.other[i] = s * 0.5;
        if (i % 50000 === 0) {
          const pct = 40 + Math.round((i / len) * 50);
          setProgress(pct);
          setProgressText(`Separando pistas... ${Math.round((i / len) * 100)}%`);
        }
      }

      setProgress(92); setProgressText('Exportando stems WAV...');

      const stemResults: StemResult[] = STEM_META.map((meta) => {
        const blob = float32ToWav(stemsData[meta.key], SAMPLE_RATE);
        const url = URL.createObjectURL(blob);
        return { ...meta, blob, url, duration: fmtDur(stemsData[meta.key].length, SAMPLE_RATE) };
      });
      ...
      setError(`Error: ${err?.message}`); setPhase('idle');
    }
  };

  const handlePlay = (idx: number) => {
    if (playingIdx !== null && audioElemsRef.current[playingIdx]) {
      audioElemsRef.current[playingIdx].pause(); audioElemsRef.current[playingIdx].currentTime = 0;
    }
    ...
    audioElemsRef.current[idx].play(); setPlayingIdx(idx);
  };

  const handleLoadAll = () => {
    if (!onStemsReady) return;
    onStemsReady(stems.map(s => new File([s.blob], `stem_${s.key}.wav`, { type: 'audio/wav' })));
  };
  ...
  const resetAll = () => {
    ...
    setFile(null); setStems([]); setPhase('idle'); setProgress(0); setProgressText(''); setError(''); setPlayingIdx(null); setServerMode(null);
  };
  ...
  return (
    <div style={S.page}>
      <div style={{ background: 'rgba(10,6,18,0.9)', borderBottom: `0.5px solid ${T.border}`, padding: '0 24px', height: '52px', display: 'flex', alignItems: 'center', gap: '16px', backdropFilter: 'blur(20px)' }}>
        ...
        <div style={{ fontSize: '14px', fontWeight: 600, color: T.text }}>Separar Stems</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: serverMode === 'server' ? T.green : serverMode === 'local' ? T.fuchsia : '#6B7280' }}></div>
          <span style={{ fontSize: '11px', color: serverMode === 'server' ? T.green : serverMode === 'local' ? T.fuchsia : T.text3 }}>
            {serverMode === 'server' ? 'Servidor RunPod activo' : serverMode === 'local' ? 'Modo local (sin servidor)' : 'Auto-detectando...'}
          </span>
        </div>
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '40px 20px' }}>
        <div style={{ textAlign: 'center', marginBottom: '36px' }}>
          ...
          <h1 style={{ fontSize: 'clamp(24px,4vw,34px)', fontWeight: 800, letterSpacing: '-0.8px', marginBottom: '10px' }}>Separar stems con IA</h1>
          <p style={{ color: T.text2, fontSize: '15px', lineHeight: 1.6 }}>Sube cualquier canción · obtienes 4 pistas separadas · listas para el mezclador</p>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.2)', borderRadius: '980px', padding: '5px 14px', fontSize: '12px', color: T.green, fontWeight: 600, marginTop: '10px' }}>
            🔒 Tu audio no se almacena en nuestros servidores
          </div>
        </div>
        ...
        {phase === 'idle' && (
          <>
              ...
              {!file ? (
                ...
                <div style={{ padding: '18px 22px', display: 'flex', alignItems: 'center', gap: '14px' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: '12px', color: T.text3, marginTop: '2px' }}>{fmtSize(file.size)}</div>
                  </div>
            ...
            {error && <div style={{ background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: '10px', padding: '12px 16px', marginBottom: '12px', fontSize: '13px', color: '#f87171' }}>⚠️ {error}</div>}

            {file && (
              <>
                <div style={{ ...S.card, marginBottom: '14px' }}>
                  <span style={S.label}>Calidad</span>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '10px' }}>
                    {QUALITY_OPTIONS.map(q => (
                      <button key={q.id} onClick={() => setQuality(q.id as any)}
                        style={{ background: quality === q.id ? 'rgba(192,38,211,0.15)' : 'rgba(255,255,255,0.03)', border: `1px solid ${quality === q.id ? T.fuchsia : T.border}`, borderRadius: '12px', padding: '14px 8px', cursor: 'pointer', textAlign: 'center', fontFamily: 'inherit' }}>
                        ...
                        <div style={{ fontSize: '11px', color: T.text3 }}>{q.desc}</div>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ ...S.card, marginBottom: '18px' }}>
                  <span style={S.label}>Recibirás estas 4 pistas</span>
        ...
        {(phase === 'decoding' || phase === 'processing') && (
          <div style={{ ...S.card, textAlign: 'center', padding: '52px 32px' }}>
            <div style={{ width: '76px', height: '76px', margin: '0 auto 22px', background: 'linear-gradient(135deg,#EC4899,#C026D3)', borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '32px', boxShadow: '0 0 36px rgba(192,38,211,0.4)' }}>🎚️</div>
            <h3 style={{ fontSize: '20px', fontWeight: 800, color: T.text, marginBottom: '6px' }}>Separando pistas con IA...</h3>
            <p style={{ fontSize: '13px', color: T.text3, marginBottom: '28px', lineHeight: 1.6 }}>
              {progressText}<br />
              <span style={{ color: serverMode === 'server' ? T.green : T.fuchsia, fontSize: '12px' }}>
                {serverMode === 'server' ? '🔌 Usando servidor RunPod' : '💻 Procesando en tu dispositivo'}
              </span>
            </p>
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
              {STEM_META.map((s, i) => (
                <div key={s.key} style={{ textAlign: 'center' }}>
                  <div style={{ width: '42px', height: '42px', borderRadius: '10px', background: progress > 35 + i * 15 ? `${s.color}33` : 'rgba(255,255,255,0.03)', border: `1px solid ${progress > 35 + i * 15 ? s.color + '66' : T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', margin: '0 auto 5px', transition: 'all 0.4s' }}>{s.icon}</div>
                  <div style={{ fontSize: '9px', color: progress > 35 + i * 15 ? s.color : T.text3, fontWeight: 600 }}>{s.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <div style={{ ...S.card, textAlign: 'center', padding: '26px', marginBottom: '16px', borderColor: 'rgba(74,222,128,0.3)' }}>
              ...
              <p style={{ fontSize: '13px', color: T.text2 }}>
                {serverMode === 'server' ? '🔌 Separados con Demucs en RunPod' : '💻 Separados localmente'}
              </p>
            </div>
              ...
              {stems.map((stem, i) => (
                <div key={stem.key} style={{ ...S.card, display: 'flex', alignItems: 'center', gap: '14px', padding: '16px 20px', borderColor: `${stem.color}33` }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: '11px', color: T.text3, marginTop: '2px' }}>stem_{stem.key}.wav · {stem.duration}</div>
                  </div>
                  <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                    <button onClick={() => handlePlay(i)} style={{ background: playingIdx === i ? `${stem.color}22` : 'rgba(255,255,255,0.04)', border: `1px solid ${playingIdx === i ? stem.color : T.border}`, color: playingIdx === i ? stem.color : T.text2, borderRadius: '8px', padding: '7px 14px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
                      {playingIdx === i ? '⏸' : '▶'}
                    </button>
                    <a href={stem.url} download={`stem_${stem.key}.wav`} style={{ background: 'rgba(192,38,211,0.1)', border: `1px solid ${T.border}`, color: T.pink, borderRadius: '8px', padding: '7px 12px', fontSize: '12px', fontWeight: 600, textDecoration: 'none', display: 'flex', alignItems: 'center' }}>⬇</a>
                  </div>
                </div>
              ))}
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {onStemsReady && (
                <button onClick={handleLoadAll} style={{ width: '100%', background: 'linear-gradient(135deg,#EC4899,#C026D3)', border: 'none', color: '#fff', padding: '16px', borderRadius: '14px', fontSize: '15px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 0 24px rgba(192,38,211,0.4)' }}>
                  🎛️ Abrir los 4 stems en el Mezclador
                </button>
              )}
              <button onClick={resetAll} style={{ width: '100%', background: 'transparent', border: `1px solid ${T.border}`, color: T.text2, padding: '14px', borderRadius: '14px', fontSize: '14px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
