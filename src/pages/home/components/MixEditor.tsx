...
export default function MixEditor({ projectId, user, uploadedFiles, onBack, onCreditsUpdate, onExport, initialPreset, reverbOn=false, delayOn=false, stereoOn=false }: MixEditorProps) {
  ...
  const handleExportMix=async()=>{
    ...
    try {
      ...
      setExportProgress(70); setExportStep('Normalizando a -20 LUFS...'); await new Promise(r=>setTimeout(r,700));
      ...
      onExport({audioBuffer:normalized,audioUrl:wavUrl,waveformPeaks:peaks,finalLufs:-20.0,presetName:(activePreset||initialPreset)?.name,iaEqPreset:iaEqPreset.name});
  ...
  if(isExporting){
    const steps=[{l:'IA EQ '+iaEqPreset.name,t:30},{l:'Compresión',t:55},{l:'Limiter',t:75},{l:'-20 LUFS',t:92}];
    return(
      <div style={C.page}><Header user={user} onLogout={()=>{}} onCreditsUpdate={onCreditsUpdate}/>
        <div style={{display:'flex',alignItems:'center',justifyContent:'center',minHeight:'80vh',padding:'24px'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:'10px'}}>
            ...
            <div style={{background:'rgba(15,10,26,0.6)',borderRadius:'10px',padding:'12px'}}>
              ...
              <div style={{display:'flex',justifyContent:'space-between',fontSize:'10px',marginBottom:'2px'}}>
                <span style={{color:'#9B7EC8'}}>Spotify</span><span style={{color:'#4ade80'}}>-14</span>
              </div>
              <div style={{display:'flex',justifyContent:'space-between',fontSize:'10px'}}>
                <span style={{color:'#9B7EC8'}}>YouTube</span><span style={{color:'#f87171'}}>-13</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{background:'linear-gradient(135deg,rgba(20,10,36,0.92),rgba(26,14,44,0.92))',border:'1px solid rgba(192,38,211,0.3)',borderRadius:'16px',padding:'16px',marginBottom:'12px',position:'relative',overflow:'hidden'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'12px',flexWrap:'wrap'}}>
            ...
            <div>
              ...
              <div style={{fontSize:'9px',color:'rgba(155,126,200,0.7)'}}>Activo en tiempo real · se exporta junto a la mezcla · -20 LUFS</div>
            </div>
