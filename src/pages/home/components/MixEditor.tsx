...
export default function MixEditor({ projectId, user, uploadedFiles, onBack, onCreditsUpdate, onExport, initialPreset, reverbOn=false, delayOn=false, stereoOn=false, onNavigate, onSwitchToDAW }: MixEditorProps) {
  ...
  // UN SOLO useEffect — evita doble inicializacion de audio
  useEffect(() => {
    if (uploadedFiles.length > 0) {
      setAllFiles(uploadedFiles);
      initializeAudioEngineWithFiles(uploadedFiles);
    }
  ...
  const initializeAudioEngineWithFiles = async (files: File[]) => {
    return initializeAudioEngineCore(files);
  };

  const initializeAudioEngine = async () => {
    return initializeAudioEngineCore(allFiles);
  };

  const initializeAudioEngineCore = async (files: File[]) => {
    try {
      ...
      const decoded = await Promise.all(files.map(async (file,i) => {
        try {
          const ab = await file.arrayBuffer();
          const buf = await audioContext.decodeAudioData(ab);
          setLoadingProgress(30+((i+1)/files.length)*40); setLoadingStep(`Procesando ${file.name}...`);
      ...
      <div style={{maxWidth:'1400px',margin:'0 auto',padding:'16px 12px 40px'}}>
        ...
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:'16px',gap:'10px',flexWrap:'wrap'}}>
          ...
          <div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>
            ...
            {onSwitchToDAW && (
              <button onClick={onSwitchToDAW} style={{...C.ghostBtn,fontSize:'12px',padding:'8px 14px',background:'rgba(168,85,247,0.1)',border:'1px solid rgba(168,85,247,0.3)',color:'#a855f7',display:'flex',alignItems:'center',gap:6}}>
                🎛️ Cambiar a DAW
              </button>
            )}
        ...
        <div style={{...C.card,marginBottom:'12px'}}>
          ...
          <div style={{background:'rgba(8,4,16,0.88)',borderRadius:'10px',padding:'8px',border:'1px solid rgba(192,38,211,0.1)'}}>
            <canvas ref={timelineCanvasRef} width={1200} height={80}
              ...
              onMouseDown={e=>{
                const canvas=timelineCanvasRef.current;if(!canvas)return;
                const rect=canvas.getBoundingClientRect();
                const seek=(clientX:number)=>{
                  const p=Math.max(0,Math.min(1,(clientX-rect.left)/rect.width));
                  handleTimelineSeek(p*duration);
                };
                seek(e.clientX);
