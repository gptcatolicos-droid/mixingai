...
const cors = {
  'Access-Control-Allow-Origin': 'https://mixingmusic.ai',
...
serve(async (req) => {
  ...
  try {
    ...
    const res = await fetch('https://api.mercadopago.com/checkout/preferences', {
      ...
      body: JSON.stringify({
        items: [{
          ...
          description: 'Acceso ilimitado al mezclador IA + IA EQ',
          quantity: 1,
          unit_price: 3.99,
          currency_id: 'USD',
        }],
    ...
    return new Response(
      JSON.stringify({ init_point: data.init_point, sandbox_init_point: data.sandbox_init_point, preference_id: data.id }),
